crate::qdrant_test_snippet!({
    ...
    let _recommendations = client
        .query(
            QueryPointsBuilder::new("{collection_name}").query(Query::new_recommend(
    ...
    let _refined = client
        .query(
            QueryPointsBuilder::new("{collection_name}")
                ...
                .query(vec![vec![0.1, 0.2], vec![0.2, 0.1], vec![0.8, 0.9]])
                .using("colbert")
                .limit(10u64),
        )
        .await?;
    ...
    let _shard_query = client
        .query(
            QueryPointsBuilder::new("{collection_name}")
                .query(vec![0.01, 0.45, 0.67])
                .shard_key_selector(
                    ShardKeySelectorBuilder::with_shard_keys(vec![ShardKey::from(
                        "shard_1".to_string(),
                    )])
